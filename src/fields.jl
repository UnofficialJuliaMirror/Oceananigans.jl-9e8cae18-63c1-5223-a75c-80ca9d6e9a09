...
@inline xindex(f, i) = i
@inline xindex(f, ::Colon) = 1:f.grid.Nx

@inline yindex(f, i) = i
@inline yindex(f, ::Colon) = 1:f.grid.Ny

@inline zindex(f, i) = i
@inline zindex(f, ::Colon) = 1:f.grid.Nz

@propagate_inbounds getindex(f::Field, i, j, k) = @inbounds getindex(f.data, xindex(f, i), yindex(f, j), zindex(f, k))
